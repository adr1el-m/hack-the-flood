...
export default function ProfilePage() {
  ...
  const [reputation, setReputation] = useState(10); // Default fallback
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(userRef, (doc) => {
      if (doc.exists()) {
        setReputation(doc.data().reputation || 10);
      }
    });
    return () => unsub();
  }, [currentUser]);
    ...
    return (
      <div className="min-h-screen bg-gov-bg flex items-center justify-center p-4">
        <div className="text-center">
          ...
          <NavLink to="/login" className="text-gov-blue font-bold hover:underline">Go to Login</NavLink>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-gov-bg flex flex-col">
      <nav className="bg-white border-b border-slate-200 px-4 py-3 flex items-center justify-between sticky top-0 z-20">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="font-bold text-slate-800 text-sm md:text-base">My Profile</h2>
          </div>
        </div>
      </nav>
          ...
          <div className="px-6 pb-6">
            ...
            <div className="flex flex-col gap-3">
              <button className="w-full py-3 bg-white border border-slate-200 rounded-xl text-slate-700 font-medium flex items-center justify-center gap-2 hover:bg-slate-50 transition-colors">
                 <Shield size={18} /> Verified Citizen Badge
              </button>
